...
pub fn mob_system(registry: &mut Registry, _engine: &mut dyn Engine) -> Option<()> {
    {
        ...
        for mut mob_entity in facade.query::<MobEntity>() {
            ...
            tilemap.cast_ray(
                mob_entity.sprite.pos.truncate(),
                player_entity.sprite.pos.truncate(),
                |visit| {
                    if visit.cell.clips {
            ...
            if mob_entity.mob.active && player_entity.player.state.is_active() {
                ...
                if let Some(path) = block_grid.astar(start.into(), end.into(), |_, blocks| *blocks)
                {
                ...
                .collisions
                .iter()
                .filter(|collision| collision.entity == mob_entity.id)
                .for_each(|collision| {
                    if let Some(other_entity) = collision.other_entity {
                        if player_entity.id == other_entity && mob_entity.mob.is_killer {
                            ...
                            registry.push(|r| {
                                r.spawn().attach(Event::PlayerCought(PlayerCoughtEvent {}));
                            });
